<?php
/*
  $Id: Wishlist Box revision 4 for Phoenix
...
class bm_wishlist {
  ...
  function execute() {
      ...
      $counter = $wishList->count_contents();
        if ($counter > 0) {
            ...
            $wishlist_box .= '<a class="list-group-item list-group-item-action';
              ...
              $wishlist_totalised = sprintf(MODULE_BOXES_WISHLIST_BOX_CART_TOTAL, $currencies->format($wishList->show_total()));
                    }
                $wishlist_box .= '<a href="' . tep_href_link('wishlist.php') . '">' . sprintf(TEXT_WISHLIST_COUNT, $counter) . '</a>';
            } else {
                $wishlist_box = '<span class="list-group-item">' . MODULE_BOXES_WISHLIST_BOX_CART_EMPTY . '</span>';
            }
    ...
    function install() {
      tep_db_query("insert into configuration (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Enable Wishlist Box Module', 'MODULE_BOXES_WISHLIST_STATUS', 'True', 'Do you want to add the module to your shop?', '6', '1', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");
      tep_db_query("insert into configuration (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, last_modified, date_added, use_function, set_function) VALUES ('Max Wishlist Box', 'MAX_DISPLAY_WISHLIST_BOX', '4', 'How many wishlist items to display in the infobox before it changes to a counter', '6', '3', now(), now(), NULL, NULL)");
    ...
    function keys() {
      return array('MODULE_BOXES_WISHLIST_STATUS', 'MAX_DISPLAY_WISHLIST_BOX', 'MODULE_BOXES_WISHLIST_CONTENT_PLACEMENT', 'MODULE_BOXES_WISHLIST_SORT_ORDER');
    }
  }
?>
